...
API_KEY="$(tr -d '\r\n' < "$CUR_PATH/token.txt")"  # 从与脚本相同文件夹中的 token.txt 文件中读取 API 密钥

# 下载配置
DOWNLOAD_FOLDER="$(tr -d '\r\n' < "$CUR_PATH/path.txt")"  # 下载文件存储路径
if [ -z "$DOWNLOAD_FOLDER" ]; then
    echo "错误：path.txt 未设置下载目录（或为空）。请在 path.txt 中指定下载目录。"
    exit 1
fi
SYNC_ON=true
...
# Helper: fetch a page, return body and set NEXT_URL to the rel="next" link if present
fetch_page() {
    local url=$1
    local hdr
    local body
    ...
    # Save headers to file (-D) and body to file (-o)
    curl -s -D "$hdr" -H "Authorization: Bearer $API_KEY" "$url" -o "$body"

    # Read body to stdout so caller can capture it
    cat "$body"

    # Extract Link header and parse rel="next"
    # Example: Link: <https://...&page=2&per_page=100>; rel="next", <...>; rel="current"
    local link_line
    link_line=$(grep -i '^Link:' "$hdr" | tr -d '\r' || true)
    if [ -n "$link_line" ]; then
        # find the URL with rel="next"
        local next_url
        next_url=$(echo "$link_line" | sed -n 's/.*<\([^>]*\)>; rel="next".*/\1/p')
        if [ -n "$next_url" ]; then
            NEXT_URL="$next_url"
        else
            NEXT_URL=""
...
sync_files() {
    ...
    # 读取 courses.txt 文件
    if [ ! -f "$CUR_PATH/courses.txt" ]; then
        echo "错误：courses.txt 文件不存在。"
        exit 1
    fi

    # 读取课程数量
    read -r num_courses < "$CUR_PATH/courses.txt"
    echo "Number of courses: $num_courses"

    # Skip the first line (number of courses)
    tail -n +2 "$CUR_PATH/courses.txt" > temp_courses.txt
